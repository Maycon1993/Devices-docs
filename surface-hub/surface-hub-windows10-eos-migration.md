...
description: How to keep using Surface Hub after Windows 10 EOL with guidance on Teams app support, migration deadlines, and upgrade options
ms.reviewer: dpandre
manager: frankbu
...
This article provides step-by-step guidance for Surface Hub customers affected by the Windows 10 Team Edition end of support (EOS). It explains available options based on your current configuration and highlights key dates, actions, and technical pathways to maintain a supported and functional device.
...
1. **Hardware upgrade**:
   - Install a **Surface Hub 3 Compute Cartridge**, which includes the Windows 11-based Microsoft Teams Rooms on Windows platform.

2. **Software migration** *(available only until October 14, 2025)*:
   - Migrate to the Microsoft Teams Rooms platform using the process outlined in the [Surface Hub 2S OS migration guide](surface-hub-2s-migrate-os.md).

3. **Convert to Windows 11 Pro or Enterprise**:
   - Use the migration process to unlock the device and install a supported Windows 11 Pro or Enterprise image.

## Option 1: Migrate to Microsoft Teams Rooms (MTR)

### Recommended for customers who:

- Want to continue using the device in a managed meeting room scenario
- Are migrating after EOS

### Migration steps:

1. Unlock the device using the process outlined in the [Surface Hub 2S OS migration guide](surface-hub-2s-migrate-os.md).
2. Apply a **BMR (bare metal recovery) image** to install MTR.

> [!NOTE]
> After migration, the device remains unlocked and can be reimaged as needed.

> [!IMPORTANT]
> The Poster + BMR migration offering is supported only through **October 14, 2025**.
...
- Devices that have already been migrated do not need to repeat the migration process.
- Post-migration, devices can be freely reimaged to MTR or other supported Windows editions.
...
- Devices not compatible with Windows 11 due to processor limitations (e.g., some Lenovo, HP, Yealink, Crestron models) may require replacement.

## Tools to assist

- Use the [Windows 11 readiness checker](https://learn.microsoft.com/en-us/windows/whats-new/windows-11-requirements) to verify upgrade compatibility.
- Use Teams Rooms Pro Management portal Room Inventory to identify devices needing action.

> [!NOTE]
> Microsoft Learn may eventually consolidate related pages under a single Surface Hub EOS/EOL summary page. A future update may include a redirect or banner linking to that page for unified lifecycle guidance.
